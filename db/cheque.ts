...
export async function createCheque(data: PendingCheque) {
  return ChequeModel.create(data);
}

export async function getPendingCheques(): Promise<PendingCheque[]> {
  return ChequeModel.find({ processed: false, completesAt: { $lte: new Date(Date.now() + 15 * 60 * 1000) } }).lean();
}

export async function completeCheque(id: string) {
