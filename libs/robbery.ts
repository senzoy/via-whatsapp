...
const recentlyRobbed = new Map<string, NodeJS.Timeout>();
...
export function isRecentlyRobbed(victimId: string): boolean {
  return recentlyRobbed.has(victimId);
}

function setRecentlyRobbed(victimId: string) {
...
async function payAndDistribute(thiefId: string, victimId: string, totalFine: number): Promise<number> {
...
async function processCaughtByPolice(pending: PendingRobbery) {
  const { Bot } = await import("../core/core.js");
  const { getMember, AddBalance } = await import("../db/mongodb.js");
