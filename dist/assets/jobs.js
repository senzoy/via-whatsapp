...
function getJobsForTier(tier) {
    const jobs = Jobs.filter((job) => job.probability >= tier.min && job.probability <= tier.max);
    if (jobs.length > 0)
        return jobs;
    return Jobs.filter((job) => job.probability <= tier.max);
}
function randomSalary(salary) {
    return Math.floor(Math.random() * (salary.max - salary.min + 1)) + salary.min;
}
function getRandomJob(xp) {
    if (xp < 0)
        throw new Error('La XP no puede ser negativa.');
    ...
    if (Math.random() < 0.08) {
        ...
        const fine = Math.min(Math.floor(baseSalary * riskEvent.finePercent), baseSalary);
    ...
    if (selectedJob.message)
        result.message = selectedJob.message;
    return result;
}
export function getJob(xp) {
