...
function spinFull(amount) {
    ...
    const near = icons[0] === icons[1] ||
        icons[1] === icons[2] ||
        icons[0] === icons[2];
    const nearJackpot = icons.filter(i => i === "💎").length === 2;
    // Ganó
    if (triple) {
...
// COMANDO
// ─────────────────────────────────────────────────────────────
export async function Slot(ctx) {
    if (!isCasinoOpen()) {
    ...
    try {
        ...
        if (amount < MIN_BET || amount > MAX_BET) {
            return send(`❌ Apuesta fuera de rango.\n` +
        ...
        if (casinoProfit < -HOUSE_RESERVE) {
            return send("🏦 El casino está en mantenimiento financiero.\n" +
                "Vuelve más tarde.");
        }
        // ─────────────────────────────────
        // COBRAR APUESTA
        ...
        if (result.win) {
            ...
            await AddCasinoResult(userId, "slot", amount, result.payout);
        }
        else {
            await AddCasinoResult(userId, "slot", amount, 0);
        }
        // ─────────────────────────────────
        // MENSAJE
        // ─────────────────────────────────
        ...
        // COOLDOWN
        // ─────────────────────────────────
        await UpdateCooldown(userId, "casino", result.win
            ? COOLDOWN_WIN
            : COOLDOWN_LOSE);
        return send(msg);
    }
        ...
        return send("⚠️ Hubo un error en la máquina.\n" +
            "Intenta nuevamente.");
    }
}
