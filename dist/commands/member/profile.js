...
export function getRank(points) {
    ...
    if (points >= 13800)
        return '🐇 Mataviejitos';
    if (points >= 13500)
        return '📖 Testigo de Jehová';
    if (points >= 13000)
        return '🍻 Parkeador';
    if (points >= 12000)
        return '📰 Reportero';
    if (points >= 11500)
        return '🏆 Challenger LP1300';
    if (points >= 11000)
        return '🦅 Águila';
    if (points >= 10800)
        return '🏛 Diputado';
    ...
    if (points >= 10100)
        return '☕ Cafetero';
    if (points >= 10000)
        return '🍺 Bebedor experto';
    if (points >= 9800)
        return '🛴 Yeyecita';
    if (points >= 9600)
        return '💸 Auxilio Economico';
    if (points >= 9500)
        return '👴 100 a los 70';
    if (points >= 9250)
    ...
    if (points >= 7700)
        return '🐦 Gavilán Pollero';
    if (points >= 7500)
        return '🔪 Pandillero';
    if (points >= 7200)
        return '👷 Trabajador del MOP';
    if (points >= 7100)
        return '🚽 Baño Público';
    if (points >= 7000)
        return '📑 Bien Cuidao';
    ...
    if (points >= 6700)
        return '🗿 Botella';
    if (points >= 6500)
        return '🧛‍♂️ Chupa sangre';
    if (points >= 6200)
        return '🚗 Uber';
    if (points >= 6000)
        return '🦌 Venado';
    if (points >= 5800)
        return '🐟 Camisa Celeste';
    if (points >= 5500)
        return '👮‍♂️ Tongo';
    if (points >= 5200)
        return '🧀 Siu-Mai';
    ...
    if (points >= 4000)
        return '🐄 Ganadero';
    if (points >= 3800)
        return '🐗 Puerco de monte';
    if (points >= 3700)
        return '🌈 Plomero';
    if (points >= 3600)
        return '🚶‍♂️ Vendedor Ambulante';
    if (points >= 3500)
        return '🐔 Gallinazo';
    if (points >= 3200)
        return '📦 Paquetero';
    if (points >= 3000)
        return '🚚 Repartidor De P.YA';
    if (points >= 2800)
        return '🎣 Pescador';
    if (points >= 2700)
        return '🏠 Techos de Esperanza';
    if (points >= 2200)
        return '🎲 Ludópata';
    if (points >= 2000)
        return '💊 Drogadicto';
...
export async function Profile(ctx) {
    ...
    const admin = await Bot.isAdmin(ctx.jid, mentioned ? mentioned : member);
    execute({
