...
// ─── Formatter ────────────────────────────────────────────────────────────────
async function formatStats(plays) {
    ...
    const gameCount = plays.reduce((acc, p) => {
        if (!p.game)
            return acc;
    ...
    const statsByLib = plays.reduce((acc, p) => {
        if (!p.lib)
            return acc;
        if (!acc[p.lib])
            acc[p.lib] = { bet: 0, payout: 0 };
    ...
    const mentions = [];
    const topLines = await Promise.all(sorted.map(async ([lib, s], i) => {
    ...
    const content = (`🎰 *Estadísticas del Casino*\n` +
        `━━━━━━━━━━━━━━━━━━\n` +
...
// ─── Comando ──────────────────────────────────────────────────────────────────
export async function Casino(ctx) {
    try {
