...
function parseAmount(str) {
    if (!str)
        return null;
    if (str.toLowerCase() === 'all' || str.toLowerCase() === 'max')
        return -1;
...
export async function Bank(ctx) {
    ...
    if (!updated)
        return send("Error al acceder al banco.");
...
export async function Deposit(ctx) {
    ...
    if (rawAmount === null)
        return send("Usa: !deposit <cantidad> o !deposit all");
    const account = await getOrCreateBanco(userId, member.level || 0);
    const availableSpace = Math.max(0, account.maxBalance - account.balance);
    ...
    if (depositAmount <= 0) {
        if (walletBalance <= 0)
            return send("No tienes dinero en tu wallet para depositar.");
        return send(`No hay espacio disponible en tu banco. Límite: $${account.maxBalance.toLocaleString('en-US')}`);
    }
    if (walletBalance < depositAmount)
        return send(`Saldo insuficiente. Tu wallet tiene $${walletBalance.toLocaleString('en-US')}`);
    if (depositAmount > availableSpace)
        return send(`Límite excedido. Espacio disponible: $${availableSpace.toLocaleString('en-US')}`);
    const newBalance = account.balance + depositAmount;
    await Promise.all([
        AddBalance(userId, -depositAmount),
        BancoModel.updateOne({ userId }, {
...
export async function Withdraw(ctx) {
    ...
    const rawAmount = parseAmount(ctx.args[0]);
    if (rawAmount === null)
        return send("Usa: !withdraw <cantidad> o !withdraw all");
    ...
    if (withdrawAmount <= 0) {
        if (account.balance <= 0)
            return send("No tienes fondos en el banco.");
        return send(`Límite diario alcanzado. Te quedan $${Math.max(0, remainingDaily).toLocaleString('en-US')} para retirar hoy.`);
    }
    if (account.balance < withdrawAmount)
        return send(`Saldo insuficiente. Tu banco tiene $${account.balance.toLocaleString('en-US')}`);
    if (withdrawAmount > remainingDaily)
        return send(`Límite diario excedido. Te quedan $${Math.max(0, remainingDaily).toLocaleString('en-US')} para retirar hoy.`);
