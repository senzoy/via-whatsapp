...
function getColor(n: number): "rojo" | "negro" | "verde" {
...
function parseBet(raw: string): ParsedBet | null {
...
function getDynamicMultiplier(baseMultiplier: number): number {
...
export async function Ruleta(ctx: CommandContext) {
    ...
    try {
        ...
        const cooldown =
            await GetCooldown(userId, "casino");
        ...
        if (
            ...
            return send(
                `❌ Apuesta mínima: $${MIN_BET.toLocaleString()}\n` +
        ...
        if (casinoProfit < -HOUSE_RESERVE) {
            ...
            return send(
                "🏦 El casino está ajustando liquidez.\n" +
                "Intenta más tarde."
            );
        }
