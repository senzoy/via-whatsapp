...
function detectSport(conditions: any[]): { emoji: string; label: string } {
  if (!conditions?.length) return { emoji: '🎫', label: 'Apuesta' };
...
function formatCondIcon(targetType?: string, metric?: string): string {
  const m = (metric || '').toLowerCase();
  const t = (targetType || '').toLowerCase();
  if (/goal/.test(m)) return '⚽';
  if (/kill|elimination/.test(m)) return '💀';
  if (/assist/.test(m)) return '🎯';
  if (/win/.test(m)) return '🏆';
  if (/card/.test(m)) return '🟨';
  if (/corner/.test(m)) return '🚩';
  if (t === 'team') return '🏟️';
  return '👤';
}
